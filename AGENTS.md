...
- Only reach for `motion/react` where CSS cannot do the job: exit animations (`AnimatePresence`), orchestrated multi-element sequences, or physics/gesture-driven values
- Try not to use `m` for things CSS handles fine: fade-ins, slide-ins, hover scale, simple scroll reveals, some sections still use reusable motion stagger animation for cards
<!-- END:component-rules -->
